...
class Dress(models.Model):
    ...
    length = models.CharField(
        max_length=50, choices=length_choices, verbose_name="Длина"
    )
    fastener_type = models.CharField(
        max_length=100, verbose_name="Тип застёжки", blank=True, null=True
    )
    fit_choices = [
        ("tight", "Облегающий"),
        ("fitted", "Приталенный"),
    ...
    rental_period = models.PositiveIntegerField(
        default=3, verbose_name="Срок аренды (дней)"
    )
...
class DressImage(models.Model):
    dress = models.ForeignKey(
        Dress, on_delete=models.CASCADE, related_name="images", verbose_name="Платье"
    )
